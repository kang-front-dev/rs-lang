...
export class AudioGame {
  ...
  audioGameHeader(className: string) {
    ...
    headerVolume.onclick = () => {
      if (this.audio.volume === 0) {
        ...
        this.audio.volume = 0;
        headerVolume.style.backgroundImage =
  ...
  renderGame(num: number) {
    ...
    function disabledBtn() {
      (
        ...
        el.disabled = true;
        if (el.textContent === questionArr[0].wordTranslate)
          el.style.background = 'rgba(126, 255, 133, 0.7)';
      });
    }
  ...
  audioStat() {
    ...
    audioStatResMainText.innerHTML = `${
      persent !== 100
        ? 'Ты можешь лучше! Повтори слова и возвращайся.'
        : 'Хорошая работа!'
    }`;
    ...
    audioStatResMain.append(
      audioStatResMainText,
      audioStatResMainBtn,
      audioStatPersentBox
    );
    audioStatViewMain.append(
      audioStatWrongText,
      audioStatWrongBox,
      audioStatRightText,
      audioStatRightBox
