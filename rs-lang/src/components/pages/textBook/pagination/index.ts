...
export class Pages extends Component {
  ...
  constructor(parentNode: HTMLElement) {
    ...
    this.groupBtnChoose = new Component(
      this.element,
      'p',
      ['textbook__subheading'],
    ...
    arr.forEach((elem: string) => {
      this.groupBtn = new Component(this.groupBtns.element, 'button', [
        'group_btn',
      ]);
      ...
      this.groupBtn.element.onclick = async () => {
        ...
        this.answerGroup = new Component(
          this.element,
          'div',
          ['answer-group'],
          answer.id
    ...
    this.containerBtnPagination = new Component(this.element, 'div', [
      'container__btn-pagination',
    ]);
    this.prevButton = new Button(
      this.containerBtnPagination.element,
      ['btn-prev'],
      'Prev',
      true
    );
    ...
    this.selectorPaginaion = new Component(
      this.containerBtnPagination.element,
      'ul',
      ['select__pagination']
    );
    ...
    arrPage.forEach((item) => {
      this.newOption = new Component(this.selectorPaginaion.element, 'li', [
        'selector-option',
      ]);
      this.newOption.element.textContent = `${item}`;

      this.newOption.element.addEventListener('click', async (event) => {
        // localStorage.setItem('page', String(arrPage.indexOf(item))); зачем
