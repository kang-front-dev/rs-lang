...
export class WordsItem extends Component {
  ...
  constructor(parentNode: HTMLElement, word: IWords) {
    ...
    const transcriptAndMusicWord = new Component(this.wordList.element, 'div', [
      'transcript-music-word',
    ]);

    //див с доп информацией об слове
    const informationWord = new Component(this.wordList.element, 'div', [
      'information-word',
    ]);

    //первй див с информацией о слове первый пример
    const informationWordExampleone = new Component(
      informationWord.element,
      'div',
      ['informationWorde-exampleone']
    );

    //второй див с информацией о слове второй пример
    const informationWordExampletwo = new Component(
      informationWord.element,
      'div',
      ['informationWorde-exampletwo']
    );

    //див в transcriptAndMusicWord со словом и транскрипцией
    const transcriptAndWord = new Component(
      transcriptAndMusicWord.element,
      'div',
      ['transcript-and-word']
    );
    const transcriptAndWordOneBlock = new Component(
      transcriptAndWord.element,
      'div',
      ['transcript-and-wordOneBlock']
    );

    //див со звком
    const soundWord = new Component(transcriptAndMusicWord.element, 'div', [
      'sound-word__div',
    ]);
    this.soundWordImg = new Component(soundWord.element, 'img', [
      'sound-word__svg',
    ]);
    ...
    this.soundWordImg.element.addEventListener('click', () =>
      soundWordAudio.play()
    );
    ...
    this.textMeaning = new Component(informationWordExampleone.element, 'div', [
      'textMeaning-word',
    ]);
    this.textMeaning.element.innerHTML = word.textMeaning;

    this.textMeaningTranslate = new Component(
    ...
    this.textExample = new Component(informationWordExampletwo.element, 'div', [
      'textExample-word',
    ]);
    ...
    const divButtons = new Component(this.wordList.element, 'div', [
      'div-buttons',
    ]);
    ...
    removeBtn.element.addEventListener('click', () => {
      if (word.id) {
        const deleteBtn = deleteUserWord(
